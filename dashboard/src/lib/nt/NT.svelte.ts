...
class NTSubscriber {
    ...
    private _value: NTType | null = $state(null);
     ...
     * Additionally, this sets the subscriber's value to `null`.
     */
    ...
    /**
     * Returns the value of the subscriber.
     */
    ...
    /**
     * Updates the subscriber's value.
    ...
    public updateValue(value: NTType | null, timestamp: number | null = null): void {
        if (timestamp === null || timestamp > (this._timestamp ?? 0)) {
            this._value = value;
 ...
 * NT Client settings.
 */
export interface NTClientSettings {
    /**
...
export class NT {
    ...
    private readonly _settings: NTClientSettings;

    private _bitrate: number = $state(0);
    private _latency: number = $state(0);
    private _serverTime: number = $state(0);
    private _serverTopics: Map<string, NTTopic> = $state(new Map());
    private _state: NTClientState = $state(NTClientState.IDLE);
    private _uri: string = $state("");
     ...
     */
    public constructor(uri: string, settings?: Partial<NTClientSettings>) {
        this._uri = uri;
        this._settings = {
        ...
        setInterval(() => {
            if (this._state === NTClientState.CONNECTED) {
                if (!this._aliveAck) return this._restart(`Timed out`);
                this._aliveAck = false;
                this._sendTimestamp();
                this._bitrate = this._usage / (NT._RTT_PERIOD / 1000);
                this._usage = 0;
            }
        }, NT._RTT_PERIOD);
    }
     ...
     */
    public connect(): void {
        if (this._state === NTClientState.IDLE) {
            this._state = NTClientState.DISCONNECTED;
            this._connect();
        }
    }

    /**
     * Gets the URI in use by the client.
     */
     ...
     */
    public getState(): NTClientState {
        return this._state;
    }

    /**
     * Gets the bitrate between the client and server (bits/s).
     */
    ...
    /**
     * Gets the latency between the client and server (ms).
     */
     ...
     */
    public getTopics(): Map<string, NTTopic> {
        return this._serverTopics;
    }

    /**
     * Subscribes to all topics under the specified prefix with the `topicsonly` flag set to `true`.
     ...
     */
    public subscribe<T extends NTType>(key: string, defaultValue: T, settings?: Partial<NTSubscriberSettings>): T | null {
    ...
    public publish<T extends NTType>(
        ...
        if (!pub) {
            ...
            if (typeof this._serverTopics.get(key)?.pubuid === `number`) pub.onAck();
    ...
    private async _connect(reconnecting: boolean = false): Promise<void> {
        if (this._state !== NTClientState.DISCONNECTED) return;
        if (this._openTimeout !== null) clearTimeout(this._openTimeout);
        this._state = NTClientState.CONNECTING;

        if (reconnecting) {
            console.log(`[NTSvelte] Waiting to reconnect...`);
            await new Promise((resolve) => setTimeout(resolve, NT._RECONNECT_DELAY));
        }
        ...
        try {
            ...
            aliveResult = await fetch(encodeURI(`http://${this._uri}:${NT._PORT}`), {
                signal: AbortSignal.timeout(NT._SERVER_AVAILABLE_TIMEOUT),
            });
        ...
        this._openTimeout = setTimeout(() => this._restart(`Timed out while connecting`), NT._OPEN_TIMEOUT);

        const wsURL = encodeURI(`ws://${this._uri}:${NT._PORT}/nt/${this._settings.appName}`);

        this._ws = new WebSocket(wsURL, [NT._WS_PROTOCOL]);
        this._ws.binaryType = `arraybuffer`;

        this._wsRtt = new WebSocket(wsURL, [NT._WS_RTT_PROTOCOL]);
    ...
    private _onOpen(rtt: boolean): void {
        ...
        if (this._ws?.readyState === WebSocket.OPEN && this._wsRtt?.readyState === WebSocket.OPEN) {
            ...
            this._openTimeout = null;
            this._aliveAck = true;
            this._state = NTClientState.CONNECTED;
        ...
        if (typeof event.data === `string`) {
            ...
            data.forEach((frame, i) => {
                ...
                switch (method) {
                    case `announce`:
                        ...
                        break;
                    case `unannounce`:
                        const topics = this._serverTopics;
                        topics.delete(params.name);
                        this._serverTopics = new Map(topics);

                        this._subscribers.get(params.name)?.onUnannounce();
                        break;
                    case `properties`:
                        const topic = this._serverTopics.get(params.name);
                        ...
                        this._serverTopics = new Map(this._serverTopics).set(params.name, this._deepFreeze({ ...topic, properties }));
                        break;
                    default:
            ...
            MessagePack.deserialize(event.data, { multiple: true }).forEach((unpacked: any[]) => {
                ...
                if (topicId === -1) {
                    ...
                    this._latency = usLatency / 1000;
                    this._serverTime = serverTimeUs / 1000;
